...
mod test {
    ...
    #[test]
    fn split_completion() {
        // No trailing separator: the command name is still being completed.
        assert_eq!(split("wq"), ("wq", "", true));
        // A trailing separator switches completion to the arguments.
        assert_eq!(split("wq "), ("wq", "", false));
        // Once arguments are present the command part is finished.
        assert_eq!(split("open foo"), ("open", "foo", false));
    }

    #[test]
    fn tokenize_quote_followed_by_text() {
        // Text immediately after a closing quote is its own token; no characters are dropped.
        assert_tokens(r#""a"b"#, &["a", "b"]);
        assert_tokens("'x'yz", &["x", "yz"]);
    }
